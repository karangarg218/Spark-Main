...
def main(topic):
    ...
    json_deserializer = JSONDeserializer(my_str,
                                         from_dict=TimeGender.dict_to_TimeGender)
    ...
    collection=get_mongo(db="kafka",collection="TimeGender")
    while True:
        try:
            # SIGINT can't be handled when polling, limit TimeGenderout to 1 second.
