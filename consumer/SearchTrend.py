...
def main(topic):
    ...
    my_schema = schema_registry_client.get_latest_version("searchTrend-value")
    # sotring the schema in my_Str
    my_str = my_schema.schema.schema_str

    json_deserializer = JSONDeserializer(my_str,
                                         from_dict=SearchTrend.dict_to_SearchTrend)

    consumer_conf = sasl_conf()

    consumer_conf.update({
        'group.id': 'SearchTrend',
    ...
    collection=get_mongo(db="kafka",collection="SearchTrend")
    while True:
        try:
            # SIGINT can't be handled when polling, limit SearchTrendout to 1 second.
