...
def main(topic):
    ...
    my_schema = schema_registry_client.get_latest_version("TimeProvince-value")
    # sotring the schema in my_Str
    my_str = my_schema.schema.schema_str

    json_deserializer = JSONDeserializer(my_str,
                                         from_dict=TimeProvince.dict_to_TimeProvince)

    consumer_conf = sasl_conf()

    consumer_conf.update({
        'group.id': 'TimeProvince',
    ...
    collection=get_mongo(db="kafka",collection="TimeProvince")
    while True:
        try:
            # SIGINT can't be handled when polling, limit TimeProvinceout to 1 second.
